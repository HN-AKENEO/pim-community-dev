...
class NonExistentChannelLocaleValuesFilter implements NonExistentValuesFilter
{
    ...
    {
        $filteredRawValues = [];
        foreach ($onGoingFilteredRawValues->nonFilteredRawValuesCollectionIndexedByType() as $type => $typeAttributeProducts) {
            $filteredRawValues[$type] = [];
            foreach ($typeAttributeProducts as $attributeCode => $products) {
                foreach ($products as $productIndex => $product) {
                    $product['values'] = $this->filterProductValues($product['values']);
                    $filteredRawValues[$type][$attributeCode][$productIndex] = $product;
                }
            }
        }
